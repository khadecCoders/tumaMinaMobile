...
const ChatContacts = () => {
    ...
    const [tempChatContacts, setTempChatContacts] = useState([]);

    useEffect(() => {
        const contRefs = ref(db, 'users/');
        onValue(contRefs, (snapshot) => {
            ...
            if (data) {
                ...
                const allConts = newCont.filter((item) => item.id != profile.userId)
                setTempChatContacts(allConts);
                setChatContacts(allConts);
            }
        });
    }, []);
  ...
  return (
    <View style={[styles.container, {backgroundColor: COLORS.surface}]}>
       <Header 
        leftView={search ? null : <LeftView1/>}
        title={viewAdd ? ' ' : 'Select Contact'} 
                ...
                <View>
                    ...
                    <FlatList
                        ...
                        <TouchableOpacity  onPress={() => {
                            ...
                            navigation.navigate('Chats',{ userName:item.username,userIds:item.userId,allChats:[],userTkn:item.expoToken})
                        }
                    }>
                            <View style={{flex: 1, flexDirection: 'row', padding: 15, width: '100%', alignItems: 'center' }}>
                               ...
                               <ProfileImage name={item.username} size='Small'/>
                                <View>
                                    <Text style={[STYLES.textHeading, {fontSize: 15, paddingVertical: 2, color: COLORS.onTertiaryContainer}]}>{item.username}</Text>
                                    <Text numberOfLines={3} style={{fontSize: 15, color: COLORS.outline}}>{item.useremail}</Text>
                                    <Text numberOfLines={3} style={{fontSize: 15, color: COLORS.outline}}>{item.usernumber}</Text>
